...
// Focus trap hook inline to make it self-contained
...
// Color palette mapping to the new resume theme (KPR-inspired yellow, bone-white, dark)
const palette = {
  stroke: "rgba(236, 234, 226, 0.15)", // subtle bone-white border
  strokeStrong: "rgba(236, 234, 226, 0.3)", // medium bone-white border
  accent: "var(--iris)", // electric yellow
  accentSoft: "rgba(232, 217, 12, 0.08)",
  emerald: "var(--periwinkle)", // bone white
  emeraldSoft: "rgba(236, 234, 226, 0.08)",
  amber: "var(--iris-bright)", // hot yellow
  amberSoft: "rgba(252, 238, 10, 0.08)",
  violet: "var(--alert)", // red-pink alert/accent
  violetSoft: "rgba(255, 46, 85, 0.08)",
  slateText: "var(--paper)", // paper off-white
  slateMuted: "rgba(236, 234, 226, 0.6)", // periwinkle with opacity
  neutralFill: "var(--world-2)", // dark secondary background
};
...
export const ProjectDiagram = ({ id, caption }: { id: string; caption?: string }) => {
  ...
  if (!Component) {
    return (
      <div className="border border-dashed border-alert/30 bg-alert/5 p-6 text-center font-mono text-xs text-alert uppercase">
  ...
  return (
    <figure className="space-y-3">
      <div className="relative overflow-hidden border border-periwinkle/15 bg-world-2/30 hover:border-iris-bright/60 transition-all duration-300">
        <button
          type="button"
          onClick={openLightbox}
          className="group relative block w-full text-left outline-none focus-visible:ring-2 focus-visible:ring-iris-bright/40 cursor-pointer"
          aria-label="Open diagram larger"
        >
          <span className="absolute right-2 top-2 z-10 border border-periwinkle/20 bg-world px-2 py-1 font-mono text-[9px] uppercase tracking-widest text-periwinkle/70 opacity-90 backdrop-blur-sm transition group-hover:border-iris-bright group-hover:text-iris-bright">
            Tap to enlarge
          </span>
      ...
      {caption && (
        <figcaption className="text-center font-mono text-xs leading-relaxed text-periwinkle/50 mt-1 uppercase tracking-wide">
          {caption}
        </figcaption>
      )}
      ...
      {lightboxOpen && mounted && createPortal(
        <div
          className={`fixed inset-0 z-[120] flex flex-col bg-world/95 p-3 sm:p-5 transition-opacity duration-200 ease-out ${
        ...
        >
          ...
          <div className={`relative z-10 flex flex-col flex-1 min-h-0 w-full transition-all duration-200 ease-out transform ${
          ...
          } border border-iris/45 p-4 sm:p-6 bg-world-2`}>
            <div className="relative z-10 mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-periwinkle/15 pb-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-mono text-[10px] uppercase tracking-widest text-periwinkle/50">Zoom</span>
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
                  ...
                  className="border border-periwinkle/30 bg-periwinkle/5 px-3 py-1 font-mono text-sm text-periwinkle hover:bg-periwinkle/15 hover:border-iris-bright hover:text-iris-bright disabled:opacity-20 transition-colors duration-200 cursor-pointer"
                  aria-label="Zoom out"
                >
                  −
                </button>
                <span className="min-w-[3rem] text-center font-mono text-xs text-iris-bright font-bold" title="100% = fit to viewer">
                  {Math.round(zoom * 100)}%
                </span>
                <button
                  type="button"
                ...
                <button
                  ...
                  className="border border-periwinkle/30 bg-periwinkle/5 px-2.5 py-1 font-mono text-[11px] text-periwinkle hover:bg-periwinkle/15 hover:border-iris-bright hover:text-iris-bright transition-colors duration-200 cursor-pointer"
                >
                  Reset
                </button>
              </div>
              <button
                ...
                className="border border-periwinkle/30 p-1.5 text-periwinkle hover:text-iris-bright hover:bg-iris-bright/15 hover:border-iris-bright transition-all duration-300 cursor-pointer"
                aria-label="Close"
              >
            ...
            <div
              ref={viewportRef}
              className="relative z-10 min-h-0 flex-1 overflow-auto border border-periwinkle/15 bg-world"
            ...
            >
              <div className="flex min-h-full min-w-full items-center justify-center box-border p-5">
                ...
                >
                  <div
                    className="border border-periwinkle/10 bg-world-2/40 shadow-xl"
                    style={
                      fitLayout
                        ? {
                            width: fitLayout.vbW,
            ...
            {caption && (
              <p className="relative z-10 mt-2 text-center font-mono text-[11px] text-periwinkle/70 uppercase tracking-wide">
                {caption}
              </p>
            )}
...
const DIAGRAMS_BY_SLUG: Record<string, { id: string; caption?: string }[]> = {
  ...
  aerovit: [
    { id: "aerovit-architecture", caption: "Layered Fitness & Web3 Architecture" },
    { id: "aerovit-dataflow", caption: "Data Flow & Reward Mechanics" }
  ],
  stickout: [
    { id: "stickout-architecture", caption: "StickOut MVC System Architecture" }
  ],
  "emg-controller": [
    { id: "emg-pipeline", caption: "Embedded EMG Signal Processing Pipeline" }
  ]
};

export const ProjectDiagramsSection = ({ slug }: { slug: string }) => {
  const diagrams = DIAGRAMS_BY_SLUG[slug];
